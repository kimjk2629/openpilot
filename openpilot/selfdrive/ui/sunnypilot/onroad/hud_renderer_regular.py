...
from openpilot.selfdrive.ui.sunnypilot.onroad.nda_indicator import NdaIndicatorRenderer
...
class HudRendererSP(HudRenderer):
  def __init__(self):
    ...
    self.nda_indicator = NdaIndicatorRenderer()
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    ...
    self.nda_indicator.render(rect)
    road_name_rect = rect
    if self.nda_indicator.visible:
      offset = self.nda_indicator.height + 8
      road_name_rect = rl.Rectangle(rect.x, rect.y + offset, rect.width, rect.height - offset)
    self.road_name_renderer.render(road_name_rect)
